...
namespace ModelParaOptExcelAddIn.ConstitiveModel
{
    public class MCC
    {
        ...
        public static string ConstitiveModelShortName = "MCC";
        public static string ConstitiveModelLongName = "Modified Cam-Clay model";
        ...
        public void ModelIni(double totale1, int number, EnumClass.TestType testType)
        {
            ...
            px = new double[number];//The yield surface intersects the p-axis to the right
            isEverP = new int[number];//Has it ever been elastoplastic
        ...
        /// /// <param name="pc">preconsolidation pressure</param>
        public void ModelCompute(double[] para, double sc, double e0, double b, double pc)
        {
            ...
            for (int i = 0; i < number - 1; i++)
            {
                ...
                if (denominatorOfF > 0.0)// can use TS
                {
                ...
                else// not use TS
                {
                    ...
                    Mf[i] = 6 * Math.Pow((Math.Sqrt(12 * (3 - M) / (M * M) * Math.Exp(-kesi[i] / (landa - kapa)) + 1) + 1), (-1));
                    Mc[i] = M * Math.Exp(-m * kesi[i]);
                    dqdS1 = (2 * S1[i] - S2[i] - S3[i]) / (2 * q[i]);
                    dqdS2 = (2 * S2[i] - S1[i] - S3[i]) / (2 * q[i]);
                    dqdS3 = (2 * S3[i] - S1[i] - S2[i]) / (2 * q[i]);
                    ...
                    dpdS11 = 1 / 3.0; dpdS22 = 1 / 3.0; dpdS33 = 1 / 3.0;
                    dqdS11 = (2 * S1[i] - S2[i] - S3[i]) / (2 * q[i]);//
                    dqdS22 = (2 * S2[i] - S1[i] - S3[i]) / (2 * q[i]);
                    dqdS33 = (2 * S3[i] - S1[i] - S2[i]) / (2 * q[i]);
                ...
                px[i] = p[i] * (1 + ((1 + x) * n[i] * n[i]) / (M * M - x * n[i] * n[i]));
                if (px[i] >= pc || isEverP[i] == 1)// elas-plastic
                {
                    isEverP[i + 1] = 1;
                }
                //elastic-plastic
                if (dfdS1 * dS1 + dfdS2 * dS2 + dfdS3 * dS3 > 0 && isEverP[i + 1] == 1)
                {
